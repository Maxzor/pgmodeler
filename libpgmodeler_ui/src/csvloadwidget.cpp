...
CsvLoadWidget::CsvLoadWidget(QWidget * parent, bool cols_in_first_row) : QWidget(parent)
{
	...
	fsel_wgt=new FileSelectorWidget(groupBox, new FileSelector{
					QFileDialog::AcceptOpen, QFileDialog::ExistingFile,
					"Load csv :","csv", "", {"text/csv","application/octet-stream"},
					{"","","Set","Cancel"},	{false, true},	false});
	delete gridLayout->replaceWidget(placeholder_fsel, fsel_wgt, Qt::FindChildrenRecursively);
	delete placeholder_fsel;
	...
	connect(fsel_wgt, &FileSelectorWidget::s_fileSelected, [&](){
		load_btn->setEnabled(!fsel_wgt->currentFileString().isEmpty());
	});
}
...
void CsvLoadWidget::loadCsvFile()
{
	...
	file.setFileName(fsel_wgt->currentFileString());

	if(!file.open(QFile::ReadOnly))
		throw Exception(Exception::getErrorMessage(ErrorCode::FileDirectoryNotAccessed).arg(fsel_wgt->currentFileString()),
