...
//		delete gridLayout->takeAt(gridLayout->indexOf(label_2));
		f_sel=new FileSelectorWidget(groupBox_3, &FSel_SQLOutput);
		delete gridLayout->replaceWidget(label_2, f_sel, Qt::FindChildrenRecursively);
		delete label_2;
//		gridLayout->addWidget(f_sel,1,0,1,3);
		...
		connect(f_sel, SIGNAL(s_fileSelected(QFileInfo*)), this, SLOT(enableDiffMode()));
		...
		connect(store_in_file_rb, SIGNAL(clicked(bool)), f_sel, SLOT(setEnabled(bool)));
...
{
	f_sel->setEnabled(store_in_file_rb->isChecked());
	...
	generate_btn->setEnabled(database_cmb->currentIndex() > 0 &&
							...
							((store_in_file_rb->isChecked() && !f_sel->currentFileString().isEmpty()) ||
...
void ModelDatabaseDiffForm::loadDiffInSQLTool()
{
	...
	if(store_in_file_rb->isChecked())
			filename = f_sel->currentFileString();
	else
	{
...
void ModelDatabaseDiffForm::saveDiffToFile()
{
	if(!sqlcode_txt->toPlainText().isEmpty())
	{
		...
		step_lbl->setText(tr("Saving diff to file <strong>%1</strong>").arg(f_sel->currentFileString()));
		step_ico_lbl->setPixmap(QPixmap(PgModelerUiNs::getIconPath("salvar")));
		import_item=PgModelerUiNs::createOutputTreeItem(output_trw, step_lbl->text(), *step_ico_lbl->pixmap(), nullptr);
		step_pb->setValue(90);
		progress_pb->setValue(100);

		output.setFileName(f_sel->currentFileString());

		if(!output.open(QFile::WriteOnly))
			captureThreadError(Exception(Exception::getErrorMessage(ErrorCode::FileDirectoryNotWritten).arg(f_sel->currentFileString()),
																	 ErrorCode::FileDirectoryNotWritten, __PRETTY_FUNCTION__,__FILE__,__LINE__));
