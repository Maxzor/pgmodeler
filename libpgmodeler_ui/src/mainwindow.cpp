...
void MainWindow::fixModel(const QString &filename)
{
	...
	if(!filename.isEmpty())
	{
		...
		model_fix_form.in_f_sel_wgt->setCurrentFileString(fi.absoluteFilePath());
		model_fix_form.out_f_sel_wgt->setCurrentFileString(fi.absolutePath() +
							GlobalAttributes::DirSeparator + fi.baseName() + QString("_fixed.") + fi.suffix());
	}
