...
ModelFixForm::ModelFixForm(QWidget *parent, Qt::WindowFlags f) : QDialog(parent, f)
{
	...
	extern FileSelector FSel_ExecutableInput, FSel_DatabaseModelOutput, FSel_DatabaseModelInput;

	delete gridLayout_2->takeAt(gridLayout_2->indexOf(placeholder_cli));
	delete placeholder_cli;
	cli_f_sel_wgt=new FileSelectorWidget(frame, &FSel_ExecutableInput);
	gridLayout_2->addWidget(cli_f_sel_wgt,0,0,1,2);

//	cli_f_sel_wgt=new FileSelectorWidget(frame, &ExecutableInput);
//	delete gridLayout_2->replaceWidget(placeholder_cli, cli_f_sel_wgt, Qt::FindChildrenRecursively);
//	delete placeholder_cli;

	in_f_sel_wgt=new FileSelectorWidget(frame, &FSel_DatabaseModelInput);
	delete gridLayout_4->replaceWidget(placeholder_input, in_f_sel_wgt, Qt::FindChildrenRecursively);
	delete placeholder_input;

	out_f_sel_wgt=new FileSelectorWidget(frame, &FSel_DatabaseModelOutput);
	delete gridLayout_4->replaceWidget(placeholder_output, out_f_sel_wgt, Qt::FindChildrenRecursively);
	delete placeholder_output;
	...
	connect(cli_f_sel_wgt, SIGNAL(s_fileSelected(QFileInfo*)), this, SLOT(enableFix()));
	connect(in_f_sel_wgt, SIGNAL(s_fileSelected(QFileInfo*)), this, SLOT(enableFix()));
	connect(out_f_sel_wgt, SIGNAL(s_fileSelected(QFileInfo*)), this, SLOT(enableFix()));
...
void ModelFixForm::hideEvent(QHideEvent *)
{
	...
	cli_f_sel_wgt->setVisible(false);
...
int ModelFixForm::exec()
{
	...
	//Show an warning if the cli command doesn't exist
	if(!fi.exists())
	{
		...
		cli_f_sel_wgt->setVisible(true);
		cli_f_sel_wgt->setVisible(true);
	}
	else
		cli_f_sel_wgt->setCurrentFileString(GlobalAttributes::getPgModelerCLIPath());
...
{
	if(!cli_f_sel_wgt->currentFileString().isEmpty())
	{
		QFileInfo fi(cli_f_sel_wgt->currentFileString());
	...
	fix_btn->setEnabled(!cli_f_sel_wgt->currentFileString().isEmpty() &&
						!in_f_sel_wgt->currentFileString().isEmpty() &&
						!out_f_sel_wgt->currentFileString().isEmpty() &&
...
void ModelFixForm::fixModel()
{
	...
	cmd=cmd.arg(cli_f_sel_wgt->currentFileString())
		.arg(fix_tries_sb->value())
		.arg(in_f_sel_wgt->currentFileString())
		.arg(out_f_sel_wgt->currentFileString());
...
void ModelFixForm::handleProcessFinish(int res)
{
	...
	if(res==0 && load_model_chk->isChecked())
	{
		...
		emit s_modelLoadRequested(out_f_sel_wgt->currentFileString());
		this->close();
	}
